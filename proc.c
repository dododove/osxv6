...
void list_remove(struct proc *proc)
{
  ...
  node_t *curr = ptable.head;
  node_t *prev = NULL;
  while(curr!=NULL){
    if(curr->proc == proc)
    {
      if(proc == ptable.head->proc)
      {
...
struct proc *remove_min()
{
  ...
  for (current = ptable.head; current != NULL; current = current->next)
  {
    ...
    if(proc->state == RUNNABLE) {
      ...
      {
      min = proc->stride_info.pass_value;
      sched_proc = proc;
      }
    }
  }
  if(i==1) {
    list_remove(sched_proc);
  }
  return sched_proc;
}
...
void update_min_pass_value()
{
  ...
  for (current = ptable.head; current != NULL; current = current->next)
  {
    if(current->proc->state != RUNNABLE) {
      return;
    }
    else
    {
      if(current->proc->stride_info.pass_value < min) 
      {
      min = current->proc->stride_info.pass_value;
      }
    }
  }
